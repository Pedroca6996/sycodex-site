...
function moveBall() {
    ...
    if (ballY <= 0 || ballY >= canvas.height - ballSize) {
        ballSpeedY *= -1;
        // Aumenta a velocidade ao quicar nas bordas superior/inferior
        ballSpeedY += Math.sign(ballSpeedY) * 0.1; 
    }

    if (ballX <= paddleWidth && ballY > paddle1Y && ballY < paddle1Y + paddleHeight) {
        ballSpeedX *= -1;
        // Aumenta a velocidade ao quicar na raquete 1
        ballSpeedX += Math.sign(ballSpeedX) * 0.1;
    }
    if (ballX >= canvas.width - paddleWidth - ballSize && ballY > paddle2Y && ballY < paddle2Y + paddleHeight) {
        ballSpeedX *= -1;
        // Aumenta a velocidade ao quicar na raquete 2
        ballSpeedX += Math.sign(ballSpeedX) * 0.1;
    }
    ...
    if (ballX < 0) {
        ...
        checkWin();
        resetBall();
        // Aumenta a velocidade quando um ponto é marcado
        increaseBallSpeed();
...
function resetGame() {
    score1 = 0;
    score2 = 0;
    // Reseta a velocidade da bola para o valor inicial
