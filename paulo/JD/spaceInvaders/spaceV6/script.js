...
let enemies = [];
let powerUps = [];
let explosions = []; // efeitos visuais animados
let score = 0;
let lives = 3;
let level = 1;
let gameOver = false;
...
function updateEnemies() {
    ...
    // Primeiro, verifica se algum inimigo bateu na parede
    ...
    // Atualiza posição de cada inimigo
...
function gameLoop() {
    ...
    if (!gameOver) {
        ...
        drawBullets();
        drawEnemies();
        drawExplosions(); // animação de explosões
        drawPowerUps();
        drawHUD();
