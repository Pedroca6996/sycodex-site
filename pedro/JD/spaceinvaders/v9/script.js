...
function getHighScores(){
  const scores = JSON.parse(localStorage.getItem("spaceInvadersHighScores")) || [];
...
function update(){
  ...
  for(let i=enemyBullets.length-1;i>=0;i--){
    ...
    if(enemyBullets[i].y>CH) enemyBullets.splice(i,1);
    else if(rectsOverlap(enemyBullets[i], player)){
  ...
  // Movimenta inimigos e dispara tiros aleatoriamente
  ...
  for(const en of enemies){
    ...
    if(Math.random()<0.002) enemyBullets.push({x:en.x+en.w/2-BULLET_WIDTH/2, y:en.y+ENEMY_HEIGHT, w:BULLET_WIDTH, h:BULLET_HEIGHT});
  }

  if(anyAlive){
    if(minX<=0 || maxX>=CW) enemyDir*=-1;
    for(const en of enemies) if(en.alive) en.x+=enemyDir*enemySpeed;
    for(const en of enemies) if(en.alive) en.y+=ENEMY_DROP*0.01; // descida gradual
  }

  // Colisões com projéteis do jogador
  for(let i=bullets.length-1;i>=0;i--){
  ...
  // Checa colisão inimigos com chão ou nave
  for(const en of enemies){
    if(!en.alive) continue;
...
function draw(){
  ...
  if(gameOver){
    ...
    ctx.font="24px Arial";
    ctx.fillText("High Scores:",CW/2,CH/2-40);
    const topScores=getHighScores();
    for(let i=0;i<topScores.length;i++){
      ctx.fillText(`${i+1}. ${topScores[i].score}`,CW/2,CH/2 + i*30);
    }
  }
}
