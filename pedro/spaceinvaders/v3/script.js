...
window.addEventListener("keydown", (e) => {
  if (gameOver) return; // bloqueia movimento após game over
...
// ---------------------------- Atualização do Jogo ----------------------------
function update() {
  if(gameOver) return; // pausa atualizações se acabou o jogo
  ...
  for(let i = bullets.length - 1; i >= 0; i--){
    ...
    for(const en of enemies){
      ...
      if(rectsOverlap(bullet,en)){
        en.alive = false;
        bullets.splice(i,1);  // remove projétil imediatamente
  ...
  for(const en of enemies){
    if(!en.alive) continue;
    // Se inimigo toca o chão
    if(en.y + en.h >= CH){
      en.alive = false;
      lives -= 1;
    }
    // Se inimigo colide com a nave
    else if(rectsOverlap(en, player)){
      en.alive = false;
      lives -= 1;
    }
  }
