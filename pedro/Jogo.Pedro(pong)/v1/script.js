...
function update() {
  ...
  if (
    ballX <= 20 + paddleWidth && // posição X da barrinha
    ballY + ballSize >= player1Y && // dentro da altura da barrinha
    ballY <= player1Y + paddleHeight
  ) {
    ballSpeedX *= -1; // inverte direção horizontal
    ballX = 20 + paddleWidth; // evita "grudar" na barra
  }
  ...
  // Se a bola sair pela esquerda ou direita → reinicia
...
function resetBall() {
  ...
  ballSpeedX = 4 * (Math.random() > 0.5 ? 1 : -1); // aleatório p/ esquerda ou direita
  ballSpeedY = 4 * (Math.random() > 0.5 ? 1 : -1); // aleatório p/ cima ou baixo
}
