...
// NOVO: variável para armazenar a pontuação
let score = 0;
...
// NOVO: desenha a pontuação no topo do canvas
function drawScore() {
  ctx.fillStyle = "#ffffff";
  ctx.font = "20px Arial";
  ...
  for (let i = bullets.length - 1; i >= 0; i--) {
    ...
    for (let j = enemies.length - 1; j >= 0; j--) {
      ...
      if (rectsCollide(a, c)) {
        bullets.splice(i, 1);
        enemies.splice(j, 1);
        score += 100; // NOVO: soma pontos a cada inimigo destruído
        hit = true;
        break; // impede que o mesmo tiro atravesse outros inimigos
      }
    }
    if (hit) break;
  }
...
function draw() {
  ...
  drawEnemies();
  drawBullets();
  drawScore(); // NOVO: exibe pontuação
}
