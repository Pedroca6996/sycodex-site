...
let player = { x: canvas.width/2-25, y: canvas.height-60, width:50, height:30, speed:7, powerTriple:false, powerTimer:0 };
let bullets = [], enemyBullets = [], enemies = [], specialEnemies = [], powerUps = [], explosions = [];
let score = 0, lives = 3, gameOver = false, level = 1;
const shootSound = new Audio("sounds/shoot.mp3");
const explosionSound = new Audio("sounds/explosion.mp3");
let rightPressed = false, leftPressed = false, shootCooldown = 0;

// Responsividade: ajustar canvas para mobile
function resizeCanvas() {
  if (window.innerWidth < 800) {
    canvas.width = window.innerWidth * 0.95;
    canvas.height = canvas.width * 0.75; // proporção 4:3
  } else {
    canvas.width = 800;
    canvas.height = 600;
  }
...
playBtn.addEventListener("click", () => {
  ...
  if(window.innerWidth <= 768) touchControls.style.display = "flex";
});
...
document.addEventListener("keydown", (e) => {
  ...
  if (e.code === "Space" && shootCooldown<=0 && !gameOver) shoot();
});
...
shootBtn.addEventListener("touchstart", () => { if(!gameOver) shoot(); });

// Criação inimigos
function createEnemies() {
  enemies = [];
  specialEnemies = [];
  for(let i=0;i<6;i++) enemies.push({ x:i*90+30, y:30, width:40, height:30, dx:1+level*0.7, points:10, color:"#f00" });
  for(let i=0;i<2;i++) specialEnemies.push({ x:i*180+60, y:70, width:40, height:30, dx:1.2+level*0.9, points:50, color:"#0ff" });
}
createEnemies();
...
function shoot(){
  if(player.powerTriple){
    ...
    bullets.push({ x:player.x+player.width/2+20, y:player.y, width:6, height:15, dy:-5 });
  } else bullets.push({ x:player.x+player.width/2-3, y:player.y, width:6, height:15, dy:-5 });
...
// Explosão
function createExplosion(x,y){ explosions.push({ x,y,radius:0,maxRadius:20 }); }
...
function update(){
  ...
  enemyBullets.forEach((b,i)=>{
    ...
    if(b.x<player.x+player.width && b.x+b.width>player.x && b.y<player.y+player.height && b.y+b.height>player.y){
      ...
      if(lives<=0 && !gameOver){ gameOver=true; saveScore(score); shootSound.pause(); explosionSound.pause(); }
    }
...
function draw(){
  ...
  if(gameOver){
    ctx.fillStyle="#111";
    ctx.fillRect(0,0,canvas.width,canvas.height);
    ctx.fillStyle="#f43f5e"; ctx.font="48px Arial";
  ...
  let powerTextY=55;
  if(player.powerTriple){ ctx.fillStyle="#facc15"; ctx.fillText("Tiro Triplo: "+Math.ceil(player.powerTimer/60)+"s",20,powerTextY); powerTextY+=25; }
  if(player.speed>7){ ctx.fillStyle="#f43f5e"; ctx.fillText("Tiros Rápidos: "+Math.ceil(player.powerTimer/60)+"s",20,powerTextY); powerTextY+=25; }
  if(player.width>50){ ctx.fillStyle="#22d3ee"; ctx.fillText("Nave Larga: "+Math.ceil(player.powerTimer/60)+"s",20,powerTextY); }
}
