...
let level = 1;
let gameOver = false;
let scoreSaved = false; // v7.0: Flag para controlar o salvamento da pontuação
...
// --- v7.0: Funções de High Score ---

/**
 * Busca as pontuações mais altas salvas no localStorage.
 * @returns {number[]} Um array de pontuações, ou um array vazio.
 */
 ...
 * @param {number} newScore - A pontuação a ser salva.
 */
function saveHighScore(newScore) {
    if (newScore === 0) return; // Não salva pontuação zero

    const highScores = getHighScores();
    highScores.push(newScore);
    highScores.sort((a, b) => b - a); // Ordena do maior para o menor
    const topScores = highScores.slice(0, 5); // Pega apenas o top 5
    ...
    if (hitEdge) {
        ...
        enemies.forEach((enemy) => {
            ...
            if (enemy.y + enemy.height >= player.y) {
                ...
                if (lives <= 0) {
                    gameOver = true;
                    scoreSaved = false; // v7.0: Prepara para salvar a pontuação na tela de Game Over
                    if (audioEnabled) {
...
/**
 * v7.0: Função atualizada para salvar a pontuação e exibir o ranking.
 */
function drawGameOver() {
    // Salva a pontuação uma única vez quando o jogo termina
    if (!scoreSaved) {
        saveHighScore(score);
        scoreSaved = true;
    }

    ctx.fillStyle = "rgba(0, 0, 0, 0.7)"; // Fundo semi-transparente para destacar o texto
    ...
    if (highScores.length === 0) {
        ctx.fillText("Nenhuma pontuação registrada ainda!", canvas.width / 2, canvas.height / 2 + 60);
    } else {
        highScores.forEach((s, index) => {
            ctx.fillText(`${index + 1}. ${s}`, canvas.width / 2, canvas.height / 2 + 60 + index * 30);
        });
    }
    
    ctx.textAlign = "start"; // Reseta o alinhamento para o padrão
}
