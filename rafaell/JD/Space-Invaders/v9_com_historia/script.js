window.addEventListener('load', function() {
    ...
    const musicPermissionScreen = document.getElementById('musicPermissionScreen');
    ...
    const backgroundMusic = new Audio('./sounds/Musicona.mp3'); 
    ...
    class Player {
        constructor() {
            ...
            this.speed = 7; this.isHit = false; this.invincibilityTimer = 0;
            this.shootCooldown = 0; this.rapidFireActive = false; this.rapidFireTimer = 0;
        }
        draw() {
            ctx.fillStyle = '#d3d3d3'; ctx.beginPath();
            ...
            if (this.isHit) {
                ...
                ctx.strokeStyle = '#ffdd00'; ctx.lineWidth = 2; ctx.beginPath();
                ctx.arc(this.x + this.width / 2, this.y + this.height / 2, this.width / 1.5, 0, Math.PI * 2);
                ctx.stroke();
            }
        }
        update() {
            ...
            if (this.isHit) { this.invincibilityTimer--; if (this.invincibilityTimer <= 0) this.isHit = false; }
            if(this.rapidFireActive){ this.rapidFireTimer--; if(this.rapidFireTimer <= 0) this.rapidFireActive = false; }
        }
        shoot() {
        ...
        hit() {
            if (!this.isHit) { this.isHit = true; this.invincibilityTimer = 120; gameState.hits++; }
        }
    ...
    class Enemy {
        constructor(x, y, type) {
            ...
            if (type === 'Sonda') { this.width = 35; this.height = 30; this.color = '#ff4500'; } 
            else if (type === 'Comando') { this.width = 45; this.height = 40; this.color = '#da70d6'; }
        }
        draw() {
    ...
    class Boss extends Enemy {
        constructor(type) {
            ...
            if (type === 1) { width = 100; height = 150; health = 100; color = '#add8e6'; } 
            else { width = 150; height = 100; health = 150; color = '#8a2be2'; }
        ...
        update() {
            ...
            if (this.shootTimer <= 0) {
                if (this.bossType === 1) {
                    for(let i = 0; i < 3; i++){ enemyBullets.push(new Bullet(this.x + (i * this.width/2), this.y + this.height, 8, 15, '#ff5555', 4)); }
                    this.shootTimer = 80;
                } else {
                     for(let i = 0; i < 5; i++){ enemyBullets.push(new Bullet(this.x + (i * this.width/4), this.y + this.height, 6, 12, '#ff00ff', 6));}
                    this.shootTimer = 60;
                }
            }
        }
    ...
    function onEnemyKilled() {
        ...
        if(!gameState.isBossFight && enemies.length === 0){
            ...
            if (gameState.currentRound === 4) { triggerBossIntro(1); } 
            else if (gameState.currentRound === 8) { triggerBossIntro(2); } 
            else { phaseEl.textContent = gameState.currentRound; setupNormalRound(gameState.currentRound); }
        }
    }
    ...
    function gameLoop() {
        ...
        if (!gameState.paused) {
            ...
            if (gameState.isBossFight) {
                ...
                let moveDown = false;
                enemies.forEach(enemy => { if ((enemy.x + enemy.width > canvas.width && enemyDirection > 0) || (enemy.x < 0 && enemyDirection < 0)) { moveDown = true; } });
                if (moveDown) { enemyDirection *= -1; enemies.forEach(e => e.y += 20); }
                enemies.forEach(enemy => {
                    enemy.update(enemyDirection * enemySpeedX);
                    enemy.draw();
                    if(Math.random() < 0.0005 * gameState.currentRound) { enemyBullets.push(new Bullet(enemy.x + enemy.width/2, enemy.y + enemy.height, 4, 8, '#ff5555', 5)); }
                });
            }
            checkCollisions();
            updateUI();
            ...
            ctx.fillStyle = "#fff"; ctx.font = "50px 'Exo 2'"; ctx.textAlign = "center";
        ...
        bullets.forEach((bullet, bIndex) => {
            if (gameState.isBossFight && boss) {
                if (isColliding(bullet, boss)) {
                    ...
                    if (boss.health <= 0) {
                        gameState.score += 25;
                        if(boss.bossType === 2){ endGame(true); } 
                        else { gameState.currentRound++; phaseEl.textContent = gameState.currentRound; boss = null; setupNormalRound(gameState.currentRound); }
                    }
                }
            } else {
                enemies.forEach((enemy, eIndex) => { if (isColliding(bullet, enemy)) { enemies.splice(eIndex, 1); bullets.splice(bIndex, 1); onEnemyKilled(); } });
            }
        });
        enemyBullets.forEach((bullet, bIndex) => { if (isColliding(bullet, player)) { player.hit(); enemyBullets.splice(bIndex, 1); } });
    }
    ...
    window.addEventListener('keydown', (e) => {
        ...
        if (e.key === 'Enter' && gameState.running && !gameState.showingBossIntro) {
            ...
            if (musicEnabled) {
                if (gameState.paused) { backgroundMusic.pause(); } 
                else { tryToPlayMusic(); }
            }
        }
    });
